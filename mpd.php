...
$match_name = isset($_GET['match_name']) ? htmlspecialchars($_GET['match_name']) : '';

// Fetch the match data
$data = fetch_json_data();
$stream_url = '';

// Find the match in the JSON data based on the match name
foreach ($data['matches'] as $match) {
    if ($match['match_name'] == $match_name) {
        // Set the stream URL (adfree_url or dai_url)
        $stream_url = $match['adfree_url'] ?? $match['dai_url'];
        break;
    }
}

// If no stream URL is found, show an error message
if (!$stream_url) {
    echo "<div class='error-message'>Error: Stream not available for this match.</div>";
    exit;
}
echo "
<html lang="en">
